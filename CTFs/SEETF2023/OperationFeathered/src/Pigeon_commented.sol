...
contract Pigeon {
    ...
    //NOTE: Needed taskpoints to promote
    ...
    function task(bytes32 codeName, address person, uint256 data) public oneOfUs {
        ...
        //NOTE: cast to address redundant
        if (address(person).balance != data) revert();

        //NOTE: Not really needed, could also jsut use data later
        uint256 points = data;

        //NOTE: Totally irrelevant
        hasBeenCollected[person] = true;
        dataCollection[msg.sender][person] = points;

        //NOTE: Only real functionality
